...
# This script is mostly derived from:
# https://github.com/ncoudray/DeepPATH/blob/master/DeepPATH_code/00_preprocessing/0b_tileLoop_deepzoom4.py
# which is Copyright (c) 2017, Nicolas Coudray, Theodoros Sakellaropoulos, and Aristotelis Tsirigos (NYU)
...
class TileWorker(Process):
    ...
    def normalize_tile(self, tile, NormVec):
        ...
        for i in range(3):
            ...
            tmp = ((Lab[:, :, i] - TileMean[i]) *
                   (newStd[i] / TileStd[i])) + newMean[i]
            if i == 0:
                tmp[tmp < 0] = 0
                tmp[tmp > 100] = 100
                Lab[:, :, i] = tmp
    ...
    def run(self):
        ...
        while True:
            ...
            associated, level, address, outfile, _, _, PercentMasked, _, _, Normalize = data
            ...
            if True:
                try:
                    ...
                    # arr = np.array(np.asarray(bw)) # unused var
                    ...
                    if avgBkg <= (self._Bkg / 100.0):
                        ...
                        if PercentMasked >= (self._ROIpc / 100.0):
                            ...
                            if Normalize != '':
                                ...
                                tile = Image.fromarray(self.normalize_tile(
                                    tile, Normalize).astype('uint8'), 'RGB')
...
class DeepZoomImageTiler(object):
        ...
        if True:
            ...
            if (self._xmlfile != '') & (self._ImgExtension != 'jpg') & (self._ImgExtension != 'dcm'):
                ...
                mask, xml_valid, Img_Fact = self.xml_read(
                    xmldir, self._xmlLabel, self._Fieldxml)
                if xml_valid == False:
                    print(
            ...
            for level in range(self._dz.level_count-1, -1, -1):
                ...
                tiledir = os.path.join("%s_files" %
                                       self._basename, str(ThisMag))
                ...
                for row in range(rows):
                    for col in range(cols):
                        ...
                        if xml_valid:
                            ...
                            Dlocation, _, _ = self._dz.get_tile_coordinates(
                                level, (col, row))
                            Ddimension = tuple([pow(2, (self._dz.level_count - 1 - level))
                                                * x for x in self._dz.get_tile_dimensions(level, (col, row))])
                            startIndY_current_level_conv = (
                                int((Dlocation[1]) / Img_Fact))
                            endIndY_current_level_conv = (
                                int((Dlocation[1] + Ddimension[1]) / Img_Fact))
                            startIndX_current_level_conv = (
                                int((Dlocation[0]) / Img_Fact))
                            endIndX_current_level_conv = (
                                int((Dlocation[0] + Ddimension[0]) / Img_Fact))
    ...
    def xml_read(self, xmldir, Attribute_Name, Fieldxml):
        ...
        # cols, rows = self._dz.level_tiles[-1]  # unused vars

        NewFact = max(ImgMaxSizeX_orig, ImgMaxSizeY_orig) / \
            min(max(ImgMaxSizeX_orig, ImgMaxSizeY_orig), 15000.0)

        # Img_Fact = float(ImgMaxSizeX_orig) / 5.0 / float(cols) # unused var
                    ...
                    if len(vertices) > 0:
                        ...
                        if NegativeROA == "0":
                            ...
                            for vertex in vertices:
                                ...
                                x = int(
                                    round(float(vertex.attributes['X'].value) / NewFact))
                                y = int(
                                    round(float(vertex.attributes['Y'].value) / NewFact))
        ...
        img = Image.new('L', (int(ImgMaxSizeX_orig/NewFact),
                              int(ImgMaxSizeY_orig/NewFact)), 0)
        ...
        # print(mask.shape)
        if Attribute_Name == "non_selected_regions":
            Image.fromarray(255-mask).save(os.path.join(os.path.split(self._basename[:-1])[
                0], "mask_" + os.path.basename(self._basename) + "_" + Attribute_Name + ".jpeg"))
        else:
            if self._mask_type == 0:
                Image.fromarray(255-mask).save(os.path.join(os.path.split(self._basename[:-1])[
                    0], "mask_" + os.path.basename(self._basename) + "_" + Attribute_Name + "_inv.jpeg"))
            else:
                Image.fromarray(mask).save(os.path.join(os.path.split(self._basename[:-1])[
                    0], "mask_" + os.path.basename(self._basename) + "_" + Attribute_Name + ".jpeg"))
        # print(mask)
...
class DeepZoomStaticTiler(object):
    ...
    def _run_image(self, associated=None):
        ...
        tiler = DeepZoomImageTiler(dz, basename, self._format, associated, self._queue, self._slide, self._basenameJPG, self._xmlfile,
                                   self._mask_type, self._xmlLabel, self._ROIpc, self._ImgExtension, self._SaveMasks, self._Mag, self._normalize, self._Fieldxml)
        tiler.run()
...
if __name__ == '__main__':
    ...
    try:
        ...
        opts.normalize = ''
        parser.error(
            "ERROR: NO NORMALIZATION APPLIED: input vector does not have the right format")
    # if ss != '':
        ...
        if opts.xmlfile != '':
            ...
            if os.path.isfile(xmldir):
                if (opts.mask_type == 1) or (opts.oLabelref != ''):
                    ...
                    xml_labels, xml_valid = xml_read_labels(
                        xmldir, opts.Fieldxml)
                    if (opts.mask_type == 1):
                        # No inverse mask
                    ...
                    for oLabel in xml_labels:
                        ...
                        if (opts.oLabelref in oLabel) or (opts.oLabelref == ''):
                            ...
                            if (opts.mask_type == 0):
                                ...
                                output = os.path.join(
                                    opts.basename, oLabel+'_inv', opts.basenameJPG)
                                if not os.path.exists(os.path.join(opts.basename, oLabel+'_inv')):
                                    os.makedirs(os.path.join(
                                        opts.basename, oLabel+'_inv'))
                            else:
                                Nbr_ROIs_ForNegLabel += 1
                                output = os.path.join(
                                    opts.basename, oLabel, opts.basenameJPG)
                                if not os.path.exists(os.path.join(opts.basename, oLabel)):
                                    os.makedirs(os.path.join(
                                        opts.basename, oLabel))
                            if 1:
                                # try:
                                DeepZoomStaticTiler(filename, output, opts.format, opts.tile_size, opts.overlap, opts.limit_bounds, opts.quality, opts.workers, opts.with_viewer, opts.Bkg,
                                                    opts.basenameJPG, opts.xmlfile, opts.mask_type, opts.ROIpc, oLabel, ImgExtension, opts.SaveMasks, opts.Mag, opts.normalize, opts.Fieldxml).run()
                            # except:
                            #	print("Failed to process file %s, error: %s" % (filename, sys.exc_info()[0]))
                        if Nbr_ROIs_ForNegLabel == 0:
                            print("label %s is not in that image; invert everything" % (
                                opts.oLabelref))
                            ...
                            output = os.path.join(
                                opts.basename, opts.oLabelref+'_inv', opts.basenameJPG)
                            if not os.path.exists(os.path.join(opts.basename, oLabel+'_inv')):
                                os.makedirs(os.path.join(
                                    opts.basename, oLabel+'_inv'))
                            if 1:
                    ...
                    try:
                        ...
                        print("Failed to process file %s, error: %s" %
                ...
                if (ImgExtension == ".jpg"):
                    print(
                        "Input image to be tiled is jpg or dcm and not svs - will be treated as such")
                    output = os.path.join(opts.basename, opts.basenameJPG)
                    try:
                        DeepZoomStaticTiler(filename, output, opts.format, opts.tile_size, opts.overlap, opts.limit_bounds, opts.quality, opts.workers, opts.with_viewer,
                                            opts.Bkg, opts.basenameJPG, opts.xmlfile, opts.mask_type, opts.ROIpc, '', ImgExtension, opts.SaveMasks, opts.Mag, opts.normalize, opts.Fieldxml).run()
                    except Exception as e:
                        print("Failed to process file %s, error: %s" %
                              (filename, sys.exc_info()[0]))
                        print(e)
